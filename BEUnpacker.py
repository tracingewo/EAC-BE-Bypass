...
def pop_from_stack_to_variable(unpacker):
    ...
    pop_place = ctypes.c_uint8(pop_place ^ (
        unpacker.get_xor_key() & 0xff)).value
    ...
    unpacker.set_xor_key(((unpacker.get_xor_key() & 0xff)
                         ^ pop_place) + (unpacker.get_xor_key() & 0xffffff00))
    # we need to align the pop place to 4 bytes for each reg, like the stack align.
    pop_place /= 4
    stack_value = unpacker.pop_dword_from_stack()
    unpacker.set_reg_value(pop_place, stack_value)
    print(
        "[+] Pop from stack to reg r{0} the value of {1}".format(pop_place, hex(stack_value)))
...
def push_dword_from_optable(unpacker):
    ...
    unpacker.set_xor_key(ctypes.c_uint32(
        unpacker.get_xor_key() + enc_key).value)
...
def do_dword_addition_on_stack_push_flags(unpacker):
    ...
    value_2 = ctypes.c_uint32(unpacker.peek_stack_dword(
        unpacker.get_stack_ptr() + 4)).value
    ...
    print("[+] Dword addition - add 2 dwords from the stack, value {0} and flags {1}".format(
        hex(new_val), hex(flags.get_binary())))
...
def push_reg_value_into_stack(unpacker):
    ...
    unpacker.set_xor_key(((unpacker.get_xor_key() & 0xff)
                         ^ reg_index) + (unpacker.get_xor_key() & 0xffffff00))
    unpacker.push_dword(unpacker.get_reg_value(reg_index / 4))
    print("[+] Reg dword push - Push reg to stack, reg index: {0}, reg value: {1}".format(
        hex(reg_index / 4), hex(unpacker.get_reg_value(reg_index / 4))))
...
def push_word_into_stack(unpacker):
    ...
    unpacker.set_xor_key((ctypes.c_uint8((unpacker.get_xor_key(
    ) & 0xff) + value)).value + (unpacker.get_xor_key() & 0xffffff00))
...
def read_from_data_push_word(unpacker):
    ...
    print(
        "[*] Byte read - Read value {0} at address {1}".format(hex(data), hex(value)))
...
def get_stack_value_at_offset(unpacker):
    ...
    print("[+] Read byte from stack write to offset - Read value from stack: {0} write it to: {1}".format(
        byte_value, hex(offset)))
...
def store_and_result_and_flags(unpacker):
    ...
    unpacker.pook_stack_dword(unpacker.get_stack_ptr(), flags.get_binary())
    print("[+] And operation(byte) - Set value {0} at stack pushed flags to stack {1}".format(
        hex(value_1), hex(flags.get_binary())))
...
def add_byte_to_stack_store_flags(unpacker):
    ...
    unpacker.pook_stack_dword(unpacker.get_stack_ptr(), flags.get_binary())
    print("[+] Addition (byte) - Pushed value: {0} to stack with flags {1}".format(
        value_1, hex(flags.get_binary())))
...
def pop_byte_to_reg(unpacker):
    ...
    # this is the index of the semi-reg inside the big reg, we need this for the mast
    ...
    reg_value = ctypes.c_uint32((reg_value & (ctypes.c_uint32(
        ~mask).value)) + ((stack_val << shifting) & mask)).value
    unpacker.set_reg_value(value / 4, reg_value)
    print("[+] Pop stack byte write to reg - Set reg {0} new value: {1}".format(
        hex(value), hex(reg_value)))
...
def get_pointer_value(unpacker):
    ...
    print(
        "[+] Set value {0} at offset {1}".format(hex(value_at_offset), hex(stack_offset)))
...
def store_and_dword_result_and_flag(unpacker):
    ...
    unpacker.pook_stack_dword(unpacker.get_stack_ptr(), flags.get_binary())
    print("[+] And operation (dword) - Put value: {0} on the stack with flags: {1}".format(
        hex(value_1), hex(flags.get_binary())))
...
def set_dword_value_at_stack(unpacker):
    ...
    unpacker.set_xor_key((unpacker.get_xor_key() & 0xffff0000) +
                         ctypes.c_uint16(((unpacker.get_xor_key() & 0xffff) - value)).value)
    unpacker.push_dword(value)
    print(
        "[+] push dword from optable - Pushed value {0} to the stack".format(hex(value)))
...
def push_dword_to_stack(unpacker):
    ...
    unpacker.push_dword(value)
    print(
        "[+] push dword from optable - Pushed dword to stack {0}".format(hex(value)))
...
def shift_right(unpacker):
    ...
    unpacker.pook_stack_dword(unpacker.get_stack_ptr(), flags.get_binary())
    print("[+]Shift right (Dword) - Pushed value: {0} with flags: {1}".format(
        hex(value), hex(flags.get_binary())))
...
def pop_optable_pointer(unpacker):
    ...
    print(
        "[*] Set the ipcode ptr to {0}".format(hex(unpacker.get_ip_opcode())))
...
def write_value_to_address_from_stack(unpacker):
    ...
    print("[*] Write dword value to address - write From stack address {0} the value of {1}".format(
...
def call_function_store_result(unpacker):
    ...
    unpacker.set_xor_key((ctypes.c_uint8((unpacker.get_xor_key(
    ) & 0xff) + opcode)).value + (unpacker.get_xor_key() & 0xffffff00))
    ...
    for i in range(opcode, 0, -1):
        value_at_stack = unpacker.peek_stack_dword(
            unpacker.get_stack_ptr() + i * 4)
    ...
    print("[*] Call function at address: {0} with params: {1}".format(
        hex(function_address), " ".join(hex(i) for i in arguments)))
...
def load_ptr_to_stack(unpacker):
    ...
    print("[*] get value from address push to stack - Pushed value {0} to stack from address {1}".format(
        hex(value_at_address), hex(read_value)))
...
def load_word_reg_into_stack(unpacker):
    ...
    print(
        "[+] Push word from reg - Pushed reg word into the stack with value {0}".format(dst_value))
...
def write_value_to_address(unpacker):
    ...
    if address < IMAGE_BASE:
        ...
        set_dword_to_address(address, struct.pack("<I", value))
    print(
        "[*] Write dword - Put From stack value: {0} at address {1}".format(hex(value), hex(address)))
    # raw_input()
...
def shld_from_stack(unpacker):
    ...
    unpacker.pook_stack_dword(unpacker.get_stack_ptr(), flags.get_binary())
    print(
        "[+] Shlead the value of {0} with flags: {1}".format(hex(dst), hex(flags.get_binary())))
...
def shrd_from_stack(unpacker):
    ...
    unpacker.pook_stack_dword(unpacker.get_stack_ptr(), flags.get_binary())
    print(
        "[+] Shread the value of {0} with flags: {1}".format(hex(dst), hex(flags.get_binary())))
...
def write_word_from_stack_to_reg(unpacker):
    ...
    print("[+] Get word from stack and push to reg - Put reg r{0} to value {1}".format(
        reg_index, hex(reg_value)))
...
def get_word_ptr_value_from_stack(unpacker):
    ...
    print("[+] Read word from address - Pushed value {0} to the stack from address {1}".format(
...
def shift_left_word_from_stack(unpacker):
    ...
    unpacker.pook_stack_dword(unpacker.get_stack_ptr(), flags.get_binary())
    print("[+] Shift left word -  Shift left value and pushed {0} value with flags {1}".format(
        hex(shift_value), hex(flags.get_binary())))
...
def shift_left_dword_from_stack(unpacker):
    ...
    unpacker.pook_stack_dword(unpacker.get_stack_ptr(), flags.get_binary())
    print("[+] Shift left dword - Shift left value and pushed {0} value with flags {1}".format(
        hex(shift_value), hex(flags.get_binary())))
...
def load_word_from_reg_to_stack(unpacker):
    ...
    unpacker.set_ip_opcode(unpacker.get_ip_opcode() + 1)
    reg_value = ctypes.c_uint8(
        reg_value - (unpacker.get_xor_key() & 0xff)).value
    ...
    unpacker.set_xor_key((ctypes.c_uint8((unpacker.get_xor_key(
    ) & 0xff) - reg_value)).value + (unpacker.get_xor_key() & 0xffffff00))
    ...
    unpacker.push_word(value)
    print("[+] Pushed word reg value into the stack {0}, reg index: r{1}".format(
        hex(value), reg_index / 4))
...
def write_byte_to_address(unpacker):
    ...
    print(
        "[*] Get address and write byte - Put value {0} at address {1}".format(hex(value), hex(address)))
...
def get_word_address_from_stack_and_push(unpacker):
    ...
    if address < IMAGE_BASE:
        ...
        value = get_word_from_address(address)
    unpacker.pook_stack_word(unpacker.get_stack_ptr(), value)
    print("[+] Get word from address - Pushed value {0} from {1} to the stack".format(
        hex(value), hex(address)))
...
def write_word_to_address(unpacker):
    ...
    print(
        "[*] Write word to address - Put value {0} at adddress {1}".format(hex(value), hex(address)))
...
def write_to_address_from_stack_dword(unpacker):
    ...
    if address < IMAGE_BASE:
        ...
        set_dword_to_address(address, struct.pack("<I", value))
    print(
        "[*] Write dword to address - Put Stack address {0} value {1}".format(hex(address), hex(value)))
    # raw_input()
...
def add_word_to_stack_pos(unpacker):
    ...
    unpacker.pook_stack_word(unpacker.get_stack_ptr() + 4, value_1)
    unpacker.pook_stack_dword(unpacker.get_stack_ptr(), flags.get_binary())
    print("[+] Addition (word) Added word to stack sum {0} flags {1}".format(
        hex(value_1), hex(flags.get_binary())))
...
def shift_left_byte_from_stack(unpacker):
    ...
    unpacker.pook_stack_dword(unpacker.get_stack_ptr(), flags.get_binary())
    print("[+] Shifted left byte, value {0} flags {1}".format(
        hex(shifted_value), hex(flags.get_binary())))
...
def nand_word_stack(unpacker):
    ...
    next_stack_value = (next_stack_value & 0xffff0000) + \
        (next_stack_value & ax_value)
    flags = ASMFlags()  # get flags to push
    ...
    unpacker.pook_stack_dword(unpacker.get_stack_ptr(), flags.get_binary())
    print("[+] Nand word into the stack, value {0} flags {1}".format(
        hex(next_stack_value), hex(flags.get_binary())))
...
def rdtsc(unpacker):
    ...
    print(
        "[+] Return to {0}".format(hex(unpacker.peek_stack_dword(unpacker.get_stack_ptr() + 44))))
...
def GetProcWrapper(unpacker, addresss_to_enc_string, address_of_handle):
    ...
    print(
        "[*] Load function {0} at address {1}").format(dst_string, hex(function))
...
def VirtualProtectWp(unpacker, old_prot, new_prot, size, address):
    print("[*] Called to Virtual Protect with argument {0} {1} {2} {3}").format(
        hex(address), hex(size), hex(new_prot), hex(old_prot))
...
def LocalAlloc(unpacker, flags, bytes):
    global HEAP_DICT
    print(
        "[*] Called LocalAlloc size {0} flags {1}").format(hex(bytes), hex(flags))
...
def set_word_to_address(ea, value):
    global DATA
    data = list(DATA)
    data[ea - (IMAGE_BASE + CODE_BASE_DIFF): ea + 2 -
         (IMAGE_BASE + CODE_BASE_DIFF)] = value
    DATA = "".join(data)
...
def set_dword_to_address(ea, value):
    global DATA
    data = list(DATA)
    data[ea - (IMAGE_BASE + CODE_BASE_DIFF): ea + 4 -
         (IMAGE_BASE + CODE_BASE_DIFF)] = value
    DATA = "".join(data)
...
class Unpacker():
    def __init__(self):
        ...
        print("[+] Init")
        self._ip = 0
        # the ip_opcode is hold by the esi, the esi is passing over the opcodes table, it holds by the esi reg.
        self._ip_opcode = 0x7DF4A + IMAGE_BASE
        self._current_opcode = 0
        # the xor key is hold by the ebx, the xor key is hold by the ebx reg
        self._xor_key = 0x47df4a
        # the stack of the program, it is looks like, semi-stack. in the Init state of the program we are pushing some variables. it holds by the ebp reg
        self._stack = CStack(PROGRAM_DATA, EPB_DEFAULT_VALUE, 52)
        ...
        self._stack.populate_stack(PROGRAM_DATA, EPB_DEFAULT_VALUE, [0, 0, 0, 0, 0, 0, MASK, 0, EPB_DEFAULT_VALUE, 2, ENTRY_POINT_ADDRESS,
                                   ENTRY_POINT_ADDRESS, 2,  ENTRY_POINT_ADDRESS, ENTRY_POINT_ADDRESS, 0x286, PEB_DEFAULT_VALUE, 0x5E736B32, 0x820B6FFC])
        ...
        # need to init the regs, it looks like we have 34 regs.
        self._regs = {}
        for i in range(0, 34):
    ...
    def decode(self):
        ...
        self._xor_key = ((self._xor_key & 0xff) ^ opcode) + \
            (self._xor_key & 0xffffff00)
        ...
        print("[+] set ip to ") + hex(address_op_code) + \
            " and opcode is: " + hex(opcode)
    ...
    def run(self):
        if self._current_opcode in OP_DICT:
            ...
            raise Exception("error, opcode {0} doesnt exists at address {1}".format(
                hex(self._current_opcode), hex(self._ip)))
        start_address = 0
